...
function getRandomProxyProvider(proxy: string | undefined, network: string = 'Base') {
    ...
    if (proxy) {
        ...
        provider = new JsonRpcProvider(proxyProvider, networkCfg, {staticNetwork: true})
    } else {
        provider = new JsonRpcProvider(rpc, networkCfg, {staticNetwork: true})
    }
    return provider
}
async function getNativeCoinPrice(coin: string): Promise<number> {
...
export {getProvider, getRandomProxyProvider, getTokenPrices, getNativeCoinPrice, checkConnection}
