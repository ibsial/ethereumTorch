...
async function estimateTx(signer: Wallet, txBody: TransactionRequest, multiplier = 1.3, needLog = true) {
    return retry(
        async () => {
            return ((await signer.estimateGas(txBody)) * parseUnits(multiplier.toString(), 3)) / 1000n
        },
        {maxRetryCount: 3, retryInterval: 10, needLog: needLog}
    )
}
async function sendTx(
    signer: Wallet,
    txBody: TransactionRequest,
    gasMultipliers = {price: 1.15, limit: 1.05},
    waitConfirmation = true,
    retryParams = {maxRetryCount: 3, retryInterval: 20, needLog: false}
) {
    let gasLimit = txBody?.gasLimit ?? (await estimateTx(signer, txBody, gasMultipliers.limit, retryParams.needLog))
